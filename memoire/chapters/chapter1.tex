...
\node[box, fill=white]    (att)  {Attaquant\\(\textit{handler})};
\node[box, fill=white,   right=of att]  (cc)   {Serveur C\&C};
\node[box, fill=white, right=1.8cm of cc]   (bots) {Botnet\\(zombies)};
\node[box, fill=white,   right=1.8cm of bots] (vic)  {Victime\\(serveur cible)};
...
\node[stp, fill=white]   (det) {1. Détection\\Anomalie};
\node[stp, fill=white, right=of det] (red) {2. Redirection\\BGP / DNS};
\node[stp, fill=white,    right=of red] (fil) {3. Filtrage\\DPI, Rate limit};
\node[stp, fill=white,  right=of fil] (rei) {4. Réinjection\\GRE / MPLS};
...
\begin{tikzpicture}[
    ...
    \node[box, fill=white]    (att) {Attaquants};
    \node[box, fill=white,   right=of att] (dns) {DNS\\Anycast};
    \node[box, fill=white, right=of dns, yshift=0.8cm]  (pop1) {PoP Europe};
    \node[box, fill=white, right=of dns]                (pop2) {PoP Asie};
    \node[box, fill=white, right=of dns, yshift=-0.8cm] (pop3) {PoP USA};
    \node[box, fill=white,   right=2.5cm of pop2] (origin) {Serveur\\d'origine};
...
Le mécanisme \textbf{RTBH} (\textit{Remote Triggered Black Hole}, RFC~5635 \parencite{kumari2009rtbh}) permet d'automatiser ce processus via \gls{bgp}~:
\begin{enumerate}
...
% --- Nœuds ---
\node[box, fill=white]   (att)     {Attaquants};
\node[box, fill=white,  right=of att]     (transit) {FAI Transit};
\node[box, fill=white,  right=of transit] (victim)  {Victime};
...
\hline
Cloudflare~\parencite{cloudflare2025}       & 348~Tbps (réseau global) & L3/L4 + L7 (WAF intégré) & Always-on (automatique) \\
\hline
AWS Shield Advanced~\parencite{awsshield2024} & Élastique (infra AWS) & L3/L4 + L7 (avec WAF) & Always-on + sur demande \\
\hline
Azure DDoS Protection~\parencite{azureddos2024} & Élastique (infra Azure) & L3/L4 & Always-on \\
\hline
Google Cloud Armor~\parencite{googlearmor2024} & Élastique (infra Google) & L7 (HTTP/HTTPS) & Always-on \\
\hline
...
Les systèmes \gls{p2p} peuvent adopter différentes formes d'organisation, dont l'efficacité varie selon les objectifs et les contraintes du système distribué \parencite{tanenbaum2017distributed}. Dans le cadre d'une infrastructure de défense collaborative, plusieurs modèles sont particulièrement pertinents.
