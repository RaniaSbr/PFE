...
Tel que défini en introduction, le système couvre exclusivement les \textbf{échanges inter-nœuds} au sein d'une coalition déjà constituée~; les diagrammes de conception ont été réalisés avec les outils draw.io~\parencite{drawio2024} et Lucidchart~\parencite{lucidchart2024} selon les conventions \gls{uml}~2.5.1~\parencite{omguml251}~: coordination collaborative, gestion de la confiance et sécurisation des communications. La détection réseau des attaques, le filtrage effectif du trafic et la configuration des tunnels d'infrastructure restent hors périmètre.
...
La Figure~\ref{fig:archi_redirection} présente l'architecture globale du système et délimite explicitement son périmètre. Le système gère l'intégralité de la coordination applicative~: diffusion des demandes d'aide (\textbf{\ding{182}}), collecte des réponses des pairs acceptants ou refusants (\textbf{\ding{183}}) et émission du signal de redirection (\textbf{\ding{184}}). En revanche, la redirection physique du trafic reste hors périmètre et repose sur l'infrastructure réseau du nœud victime (routeur, tunnel GRE/VXLAN/IPIP)~: c'est elle qui achemine le trafic excédentaire vers les pairs aidants (\textbf{\ding{185}}), lesquels filtrent le trafic malveillant et renvoient le trafic propre vers cette même infrastructure (\textbf{\ding{186}}), avant qu'il ne soit agrégé et retransmis au nœud victime (\textbf{\ding{187}}).
...
\begin{figure}[H]
\centering
\figborder[\textwidth]{figures/uc_mitigation_collaborative.pdf}
...
Le groupe \textbf{Membership} couvre le cycle de vie complet du pair~: il s'enregistre via {POST /peers/register}, action qui inclut obligatoirement la \textbf{publication de ses capacités de scrubbing} ({POST /capability/advertise})~; il peut ensuite \textbf{découvrir des pairs disponibles} ({POST /peers/discover}) et \textbf{consulter la liste des pairs connus} ({GET /peers})~; il envoie un \textbf{heartbeat} ({POST /heartbeat}) de façon \textbf{événementielle} — lors de son adhésion ou de toute mise à jour de ses paramètres — pour signaler sa disponibilité et sa capacité déclarée, sans minuteur périodique~; enfin, il peut \textbf{quitter proprement la coalition} ({POST /peers/goodbye}), ce qui met son statut à {INACTIVE} ou {MAINTENANCE}.
...
\hline
CU~1 & Obtenir un token JWT RS256      & Nœud local / pair      & Authentification auprès de l'API via une clé privée RS256~; token valable 15~min. \\
\hline
CU~3 & Envoyer un heartbeat            & Nœud pair              & Signal événementiel (adhésion, mise à jour, départ) transmettant la charge et la capacité disponible. \\
\hline
...
\textbf{ID} & 1 \\
\hline
\textbf{Description} & Le nœud (local ou pair) demande un token JWT signé RS256 afin d'accéder aux routes protégées de l'API. Le token est valide 15~minutes. \\
\hline
...
\textbf{3.}\ Le système génère un JWT signé contenant {\{iss, node\_id, exp\}} avec une durée de vie de 15~minutes.\newline
...
\textbf{ID} & 3 \\
\hline
\textbf{Description} & Un nœud pair signale sa disponibilité et met à jour sa capacité déclarée auprès du nœud local, de façon \textbf{événementielle} — sur adhésion à la coalition ou sur changement de ses paramètres — et non selon un intervalle périodique fixe. \\
\hline
...
\hline
\textbf{Enchaînement principal} &
Ce CU est déclenché par l'un des trois événements suivants~: (a) adhésion initiale du pair à la coalition, (b) mise à jour de ses paramètres locaux (capacité de filtrage, charge déclarée), ou (c) départ de la coalition (cf. CU~2, {POST /peers/goodbye}). Il n'existe pas de minuteur déclenchant ce signal automatiquement.\newline
\textbf{1.}\ Le pair envoie {POST /api/v1/heartbeat} avec~: {peer\_id}, son statut ({reported\_status}), la charge actuelle ({reported\_load\_pct}), la capacité disponible ({reported\_available\_gbps}) et la latence aller-retour mesurée ({round\_trip\_time\_ms}).\newline
\textbf{2.}\ Le nœud local met à jour le champ {last\_heartbeat} du pair et ses métriques (capacité, charge, latence).\newline
\textbf{3.}\ Le nœud local retourne HTTP~200 avec le statut courant du pair. \\
\hline
\textbf{Post-conditions} & Le champ {last\_heartbeat} est mis à jour~; la capacité disponible reflète l'état déclaré le plus récent du pair, jusqu'au prochain événement. \\
\hline
\textbf{Enchaînements alternatifs} &
...
\textbf{Pair injoignable lors d'une sollicitation ultérieure~:} en l'absence de surveillance périodique, un pair silencieux n'est pas détecté proactivement~; il est marqué {INACTIVE} de façon réactive si une demande d'aide ultérieure ({POST /help/request}, CU~4) échoue à le joindre. \\
\hline
\end{tabular}
\caption{Documentation du cas d'utilisation \textit{Envoyer un heartbeat (événementiel)}.}
\label{tab:cu3}
\end{table}
...
\textbf{4.}\ Le nœud victime envoie {POST /api/v1/help/request} à l'ensemble des pairs retournés par la sélection WSM (étape~3), chacun recevant une allocation proportionnelle à son score ({allocation\_pct}).\newline
\textbf{5.}\ Une session d'aide est créée côté pair avec le statut {REQUESTED}. \\
\hline
\textbf{Post-conditions} & Des sessions d'aide en statut {REQUESTED} existent chez les pairs sélectionnés. \\
\hline
\textbf{Enchaînements alternatifs} &
...
\hline
\textbf{Préconditions} & Une session d'aide en statut {REQUESTED} est associée au pair~; le pair est authentifié (CU~1). \\
\hline
\textbf{Enchaînement principal} &
...
\hline
\textbf{Enchaînements alternatifs} &
\textbf{Session déjà traitée (statut non {REQUESTED})~:} le nœud retourne HTTP~409 (Conflict).\newline
...
Les quatre critères sont définis comme suit~: $C_p = \dfrac{\text{cap\_disp}(p)}{\max_j(\text{cap\_disp}(j))}$ est la capacité disponible normalisée~; $L_p' = 1 - \dfrac{L_p - L_{\min}}{L_{\max} - L_{\min}}$ est la latence normalisée inversée (une latence faible donne un score élevé)~; $T_p$ est le score de confiance PeerTrust du pair $p$~; et
\\[0.3cm]
$R_p = \dfrac{\text{crédits\_reçus}}{\text{crédits\_reçus} + \text{crédits\_donnés} + \varepsilon}$ est la réciprocité bilatérale, avec $\varepsilon = 10^{-6}$ pour éviter la division par zéro.
\subsubsection{Pondération AHP}

Les poids $w_C, w_L, w_T, w_R$ ont été déterminés par la méthode \gls{ahp} de Saaty \parencite{saaty1980ahp}. La matrice de comparaison par paires (Tableau~\ref{tab:ahp}) exprime les préférences relatives entre critères~; la cohérence est vérifiée par le \textbf{ratio de cohérence} CR~$= 1{,}6\,\% < 10\,\%$, ce qui valide les poids retenus.
...
La Figure~\ref{fig:activite} illustre le flux d'activités d'une session de mitigation collaborative, réparti en deux couloirs~: le \textbf{nœud victime} et le \textbf{nœud pair}. Dès la détection d'une attaque \gls{ddos}, le nœud victime sélectionne les pairs candidats via l'algorithme \gls{wsm}. Si aucun pair n'est disponible, le nœud gère l'attaque seul. Dans le cas contraire, une demande d'aide est envoyée (\texttt{POST /help/offer})~; le nœud pair vérifie uniquement si le demandeur est banni (\texttt{BANNED})~: si c'est le cas, la demande est rejetée avec un code 403~; sinon, une session locale est créée avec le statut \texttt{OFFERED} et une réponse 201~Created est retournée. Le nœud pair peut ensuite accepter la session via \texttt{PUT /help/\{id\}/accept} (statut \texttt{ACCEPTED}) ou la rejeter via \texttt{PUT /help/\{id\}/reject} (statut \texttt{REJECTED}). Le nœud victime active alors la redirection du trafic (\texttt{POST /traffic/redirect}), qui enregistre le type de tunnel et le volume en base de données. À la fin de l'attaque, le nœud victime notifie la clôture (\texttt{POST /attack/over})~; les sessions passent au statut \texttt{COMPLETED} et les crédits de réciprocité ainsi que les scores de confiance sont recalculés.

\begin{figure}[H]
\centering
\figborder[\textwidth]{figures/activity_diagram.pdf}
...
% --- DS2 ---
\subsubsection{DS2 — Session de mitigation collaborative (CU4 + CU5 + CU6)}

La Figure~\ref{fig:seq_mitigation} couvre le flux complet d'une session de mitigation. Le nœud victime déclenche la sélection WSM (\texttt{POST /trust/select-peers}), reçoit la liste triée par score décroissant, puis envoie une requête d'aide à chaque pair sélectionné. Le pair A accepte (capacité suffisante) et le pair B rejette (capacité insuffisante). Une fois la session acceptée, la redirection du trafic est activée via \texttt{POST /traffic/redirect}. À la fin de l'attaque, \texttt{POST /attack/over} clôture la session, met à jour les ledgers de réciprocité des deux nœuds et déclenche le recalcul PeerTrust (DS3).

\begin{figure}[H]
\centering
\figborder[0.95\textwidth]{figures/seq_mitigation.pdf}
...
\begin{figure}[H]
\centering
\figborder[0.7\textwidth]{figures/seq_peertrust.pdf}
...
Au-dessus de mTLS, chaque requête \gls{api} porte un \gls{jwt} \parencite{rfc7519jwt} signé en RS256 \parencite{rfc7518jwa}. L'architecture asymétrique choisie présente des avantages importants par rapport à un HMAC partagé. Elle garantit d'abord l'\textbf{absence de secret partagé}~: chaque nœud signe avec sa propre clé privée, les autres vérifiant avec sa clé publique (stockée dans {PEERS.public\_key}), de sorte que la compromission d'un nœud n'expose pas les autres. Elle assure également la \textbf{non-répudiation}~: la signature RS256 lie le token à l'identité du nœud émetteur. Enfin, les tokens ont une \textbf{durée de vie limitée} de 15 minutes, ce qui limite la fenêtre d'exploitation en cas d'interception.
