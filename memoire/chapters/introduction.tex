...
Ce travail se concentre exclusivement sur les \textbf{échanges inter-nœuds} au sein d'une coalition de défense déjà constituée. Plus précisément, le périmètre couvre~:

\begin{itemize}
    \item la conception et l'implémentation de l'\textbf{API REST} exposée par chaque nœud pour la coordination collaborative~;
    \item le \textbf{protocole d'aide} entre nœuds~: signalement d'attaque, demande et acceptation de sessions de mitigation, redirection de trafic, clôture~;
    \item le \textbf{mécanisme de confiance} fondé sur PeerTrust~: calcul des scores, détection des violations, exclusion automatique~;
    \item la \textbf{sécurisation des communications} inter-nœuds par mTLS et JWT RS256.
\end{itemize}

Les aspects suivants sont explicitement \textbf{hors périmètre}~:

\begin{itemize}
    \item la formation du réseau P2P (bootstrap, découverte initiale des pairs, protocoles de routage overlay)~;
    \item la détection technique des attaques DDoS au niveau réseau (analyse de trafic, deep packet inspection)~;
    \item le filtrage et le nettoyage effectif du trafic (scrubbing) au sein de chaque nœud~;
    \item la gestion des tunnels de redirection au niveau infrastructure (configuration GRE, BGP Flowspec).
\end{itemize}
...
\begin{itemize}
    ...
    Ce chapitre présente les fondements théoriques des attaques \gls{ddos}, la taxonomie des vecteurs d'attaque et les mécanismes de mitigation existants (\textit{scrubbing centers}, CDN, pare-feu, RTBH, services cloud), ainsi que les principes des architectures pair-à-pair applicables à la coordination inter-n\oe{}uds.

    \item \textbf{Chapitre~2~: État de l'art.}
    Ce chapitre analyse les principales approches collaboratives et distribuées de défense contre les attaques \gls{ddos}~: le protocole de signalisation \gls{dots}, la DDoS Clearing House, la mutualisation opérationnelle NaWas/NBIP et l'overlay coopératif DefCOM. Une synthèse comparative identifie les lacunes motivant la contribution de ce mémoire.

    \item \textbf{Chapitre~3~: Conception et Architecture du système.}
    Ce chapitre décrit les décisions de conception du système~: topologie \gls{p2p} de la coalition, modèle de données, mécanisme de confiance fondé sur PeerTrust, algorithme de sélection des pairs par \gls{wsm} avec pondération \gls{ahp}, protocoles de sécurité mTLS et JWT RS256, et interface \gls{api} \gls{rest}.

    \item \textbf{Chapitre~4~: Implémentation et Évaluation.}
    Ce chapitre présente la réalisation concrète du prototype~: pile technologique, implémentation du gestionnaire de confiance et du sélecteur de pairs, configuration de la sécurité, et résultats des tests de scalabilité conduits sur une coalition simulée jusqu'à 100 nœuds.
\end{itemize}
